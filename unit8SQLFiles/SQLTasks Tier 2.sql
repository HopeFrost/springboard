...
CASE WHEN monthlymaintenance < 100 THEN 'cheap'
	WHEN monthlymaintenance > 100 THEN 'expensive' END AS price
FROM Facilities;
...
SELECT DISTINCT CONCAT_WS(' ', m.firstname, m.surname) AS name, fac.court 
FROM Bookings AS b
LEFT JOIN Members AS m USING(memid)
INNER JOIN (SELECT facid, name AS court FROM Facilities
			WHERE name LIKE "%Court%") AS fac USING(facid)
ORDER BY name;
